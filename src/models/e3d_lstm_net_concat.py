...
def rnn(images, real_input_flag, num_layers, num_hidden, configs):
  ...
  with tf.variable_scope('generator'):
    ...
    for time_step in range(total_length - 1):
      with tf.variable_scope('e3d-lstm', reuse=reuse):
        ...
        # concat hidden state from both layers to feed into decoder
        concat_hidden = tf.concat(hidden, axis=-1, name='concat')

        print('concat_hidden shape:', concat_hidden.shape)

        # reshape hidden state from 4dim to 5dim for prediction (batch_size, length, width, height, channels)
        hidden_state_clf = tf.reshape(concat_hidden,
                                             [batch_size, window_length, ims_width, ims_height, 64*len(hidden)])
        print('hidden_state_clf shape:', hidden_state_clf.shape)
  ...
  l2_loss = tf.sqrt(tf.nn.l2_loss(gen_images - images[:, 1:]) * 2)
